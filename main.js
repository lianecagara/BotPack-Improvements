...
function onBot() {
  ...
  login(loginData, async (err, api) => {
        ...
        for (const command of listCommand) {
          try {
            ...
            if (!config?.commandCategory) {
              try {
                throw new Error(
                  `[ COMMAND ] ${command} commandCategory is empty!`,
                );
              } catch (error) {
                console.log(chalk.red(error.message));
                continue;
            ...
            try {
              ...
              console.error(
                "An error occurred while loading the command:",
                err,
              );
            }
    ...
    if (api.bot) {
      api.bot.on("ws_open", () => {
        let r = [];
        for (const key of global.client.commands.keys()) {
          const { config } = global.client.commands.get(key);
          r.push({
            name: config.name,
            description: config.description,
          });
        }
        api.bot.addCommands(global.config.PREFIX, r);
      });
    }
